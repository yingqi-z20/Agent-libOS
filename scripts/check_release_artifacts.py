...
def _validate_long_description_metadata(
    metadata: object,
    *,
    artifact: str,
    version: str,
    ...
    if metadata["Description-Content-Type"] != "text/markdown":
        raise ValueError(f"{artifact} long description must be Markdown")
    payload = metadata.get_payload()
    if not isinstance(payload, str):
        raise ValueError(f"{artifact} long description must be text")
    if "Generated by scripts/generate_pypi_readme.py" not in payload:
        raise ValueError(
            f"{artifact} long description must use the generated PyPI README"
        )
    mutable_prefix = f"{REPOSITORY_URL}/blob/main/"
    if mutable_prefix in payload:
        raise ValueError(
            f"{artifact} long description contains mutable main-branch links"
        )
    versioned_prefix = f"{REPOSITORY_URL}/blob/v{version}/"
    if versioned_prefix not in payload:
        raise ValueError(
            f"{artifact} long description must link to immutable tag v{version}"
        )
...
def _validate_wheel(wheel_path: Path, version: str) -> None:
    ...
    with zipfile.ZipFile(wheel_path) as archive:
        ...
        _validate_long_description_metadata(
            metadata,
            artifact="wheel",
            version=version,
        )
