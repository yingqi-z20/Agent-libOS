...
- `modeled` validates catalog structure and utility/security claims only. It
  contributes no tool calls, external-effect ids, operations, or runtime
  coverage.
...
| `modeled_fallback` | Native scenarios reclassified as modeled; v1 has no fallback path and requires this to be `0`. |
