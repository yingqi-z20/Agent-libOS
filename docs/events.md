...
Agent libOS 1.5.3 persists a closed catalog of `EventType` values. Events
...
| `tool_batch_truncated` | LLM executor settles a multi-call response whose later calls did not run (a failed call, queued process input, a wait, or an exec) | `llm.executor` → PID | `stop_reason`, `requested_count`, `executed_count`, `unexecuted_actions` (tool names only), `instruction` | `high` |
