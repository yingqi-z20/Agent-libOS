...
| SQLite RuntimeStore | Default local backend, file and in-memory targets | Ubuntu deterministic lanes plus the Windows 3.11 complete deterministic lane | macOS filesystem ACL and locking behavior still needs a native release-gate run; platform CI is not a proof against hostile local administrators |
...
| PTY Runtime Module | POSIX PTY plus optional Windows `pywinpty`/ConPTY session I/O; source checkout/source distribution only, not the core wheel | POSIX paths on Ubuntu; Windows 3.11 installs the `pty` extra before running the complete deterministic lane | Current Windows backend has no Job Object, parent-death containment, or wall/CPU/RSS supervision; budgeted `SubprocessLimits` spawns fail closed. Native CI coverage does not expand those implementation guarantees |
| Typed Git provider | System Git 2.26+, fixed non-bare workspace repository; local operations, managed worktrees, patch Objects, existing remotes, and repository-local simulated PRs | Deterministic provider/security/runtime tests use temporary SHA-1/SHA-256 repositories and local bare remotes on Ubuntu, with the complete deterministic lane also running on Windows 3.11; Shell/PTY/provenance hardening is parameterized | Credential-manager integrations and real HTTPS/OpenSSH authentication require environment-gated runs; GitHub/GitLab APIs are not implemented |
...
version; Windows Python 3.11 complete deterministic lane with Deno and the PTY
extra; GUI lane; isolated AgentDojo Python 3.11/3.12 harness; PostgreSQL job;
