...
| Semantic approval/data identification | Default-off, Shadow-only broker, durable job/evidence plane, three capture classes, scripted/deterministic adapters, and optional dedicated external profile | Deterministic unit/runtime/security/provider tests cover strict models, authority ceiling, broker, queue/CAS, privacy, read-only surfaces, and fake provider failures | No machine settlement or label writeback is implemented. Real classifier smoke is opt-in and not a safety oracle; production calibration, tenant rollout, Windows/PostgreSQL environment results must be recorded where actually run |
...
| Semantic GUI panel | Same-build strict decoder/component/client tests cover GET-only schema-v2 status with complete status/domain maps and strict `0/0/null`, schema-v1 detail/keyset pages, process filters, and payload rejection | Usability/calibration interpretation and native accessibility review remain operator/environment gates; semantic history is not part of the snapshot |
