...
This is the task-oriented guide to CLI workflows, output contracts, and
security-sensitive choices. It intentionally does not hand-copy every parser
argument. Use the [machine-generated exhaustive CLI reference](cli_reference.md)
or the same installed version's `agent-libos ... --help` output for the complete
command, option, positional-argument, and parser-default inventory. The checked-in
reference is regenerated by `scripts/generate_cli_reference.py` and must remain
...
The [generated CLI reference](cli_reference.md) is the checked-in exhaustive
inventory for this source version. At runtime, use
`uv run agent-libos <command> --help` for that installed parser's options and
subcommand list; for nested groups, continue with
`uv run agent-libos <group> <subcommand> --help` for the leaf parameters. The
narrative sections below document workflows and security contracts; lists
labelled “useful options” are intentionally not exhaustive.
...
This section summarizes CLI-facing package behavior. For a reusable package
layout, manifest field guidance, prompt modes, declared authority, private
workspace seeds, validation, registration, and checkpoint-derived images, use
the [AgentImage Authoring Guide](agent_images.md).

`images validate` performs package, manifest, path, schema, and static JIT
checks without registering the artifact, booting a process, invoking an LLM, or
requiring Deno. `images register` persists the validated immutable package;
reusing an image id is rejected unless the Host explicitly supplies `--replace`.

Process boot is a separate phase. It checks exact required Runtime Modules,
materializes any declared private workspace, issues only its package-scoped
workspace grants, and validates/registers packaged JIT tools through ToolBroker
before making them visible. A package with JIT tools therefore needs usable Deno
at boot even though static validation and registration do not. External
`required_capabilities` remain declarations and are never boot grants.

An Image's `default_tools` is exact, its model projection may be narrower, and
its `llm_profile` is only a Host-configured profile id. Keep credentials and
general workspace authority outside the package.
