...
Durable Task Runs, introduced in Agent libOS 1.1.0, remain the 1.3.3
...
Run metadata and hashes are persisted in store schema v4. Resuming useful work
...
recovery remain assembled so an existing v4 store cannot evade reconciliation
...
purge is a Python Host/admin surface in 1.3.3; it is not offered to ordinary
CLI or GUI users.
...
Host/admin-only explicit payload purge remains Python-only in 1.3.3):
...
separate Task Run requirements or wait HTTP route in 1.3.3: the detail response
