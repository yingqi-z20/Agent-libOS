...
- Hierarchical process resource budgets for tool calls, LLM token usage,
  ...
  one transaction. Every logical LLM call durably reserves one call and its
  configured token envelope before Provider dispatch; known usage settles
  exactly, certified non-start releases, and ambiguous outcomes charge the
  aggregate maximum before model-selected tools run.
- A loopback Electron GUI Provider trace view that groups each logical LLM call
  with the bounded network attempts observed by the built-in client. It shows
  only Provider-returned reasoning/summary/thinking that remains readable under
  local retention; it neither synthesizes explanations nor claims access to a
  model's hidden chain of thought. Snapshot/SSE traffic is summary-only and
  readable content is fetched on demand for the selected process.
...
uv pip install --python /tmp/agent-libos-wheel-check/bin/python \
  --no-deps dist/agent_libos-1.3.3-py3-none-any.whl
...
uv pip install --python /tmp/agent-libos-sdist-check/bin/python \
  --no-deps --no-build-isolation dist/agent_libos-1.3.3.tar.gz
...
Agent libOS 1.3.3 creates and opens only RuntimeStore schema v4. It rejects a
