...
unknowable; `observed_omitted_matches` counts only captured matches excluded by
`max_results`. `swe_submit` only prepares a status, summary, tests, and
residual-risks payload. It never exits the process itself: use its returned
`process_exit_args` with the built-in `process_exit` tool so the active image's
