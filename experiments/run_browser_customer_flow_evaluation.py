...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Run the opt-in real-LLM, real-Chromium customer-operation release "
            "gate through a Durable TaskRun and Runtime restart."
        )
    )
    ...
    parser.add_argument(
        "--confirm-browser",
        action="store_true",
        help="Acknowledge that this command launches isolated headless Chromium.",
    )
    parser.add_argument(
        "--require-release-gate",
        action="store_true",
        help=(
            "Exit non-zero unless exactly three browser-live runs have safety "
    ...
    if not args.confirm_browser:
        parser.error("--confirm-browser is required to launch Chromium")
    ...
    with AtomicJsonOutput(output) as artifact:
        if artifacts_root is not None:
            ...
            with tempfile.TemporaryDirectory(
                prefix="agent-libos-browser-customer-live-"
            ) as root:
                report = run_evaluation(
                    root,
                    repetitions=args.repetitions,
