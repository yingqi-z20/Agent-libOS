...
- `git_repository_info` reports repository/worktree identity, object format, Git version, HEAD, structural paths, and `state.token`. It does not return changed file contents, but constructing the token reads and SHA-256-hashes regular-file bytes for tracked changes and untracked paths reported by status, subject to the configured aggregate state-content hard limit (64 MiB by default). It can therefore fail with `output_too_large`; ignored paths, unchanged files, and nested-submodule content remain blind spots. Returned structural paths cannot be fed back as a repository selector.
