...
- An **initial state-token CAS** `stale_state`, before the first state-changing provider effect, means this invocation did not accept the old observation. Re-read status and both diffs, account for user/concurrent changes, and reconstruct the path/index decision. An unqualified or later `stale_state` is not this certificate and can follow a successful Host command.
...
- The model-facing error does not expose every provider settlement detail. After any error not clearly known to be pre-dispatch validation, permission/approval, or an **initial** state-token CAS rejection, assume stage, unstage, or commit may already have happened. This includes post-dispatch `stale_state`, plain `command_failed`, timeout, and `unknown_effect`; tool-level `retryable` never authorizes blind replay. Inspect HEAD OID, status, and both diffs first.
