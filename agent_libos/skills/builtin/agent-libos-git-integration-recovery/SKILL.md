...
The Runtime performs a protected repository read containing an internal bounded dry-run and candidate manifest before authorization, then recomputes that manifest immediately before dispatch. Clean therefore requires repository `read` in addition to its mutation rights. Candidate drift yields `stale_state`. The manifest is a conservative `ls-files` reconciliation set, not an exact deletion list: for example, files below an untracked directory can be listed even when `directories=false` leaves that directory intact. The digest/count in approval evidence are not a substitute for the caller inspecting status and literal paths. Success details include preview and candidate hashes/count, while `changed_paths` supplies reconciliation paths and does not prove every path was removed.
...
- Only an **initial state-token CAS** `stale_state` before the first state-changing provider effect certifies that this invocation did not mutate Git. Re-read and re-decide; for clean, an initial stale can specifically mean candidate bytes/types changed after approval. An unqualified or later `stale_state` is possibly dispatched. Never replace only the token.
- `repository_busy` requires a later fresh observation, not repeated mutation calls.
- A pre-dispatch Human approval pause is the only case where exact replay of unchanged parameters/token is expected. The model-facing error omits some provider settlement details, so any error not clearly known to be pre-dispatch validation, permission/approval, or an **initial** state-token CAS rejection—including a later `stale_state` or ordinary `command_failed`—must be treated as possibly dispatched and reconciled before another mutation.
