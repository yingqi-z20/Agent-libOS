...
Force and deletion require remote delete/admin authority and mandatory one-use Human approval. All pushes require remote write and repository read authority, and data-flow policy applies to the selected local commit/ref lineage. Push can perform more than one separately protected local read during lineage and dispatch preflight, so a one-use repository-read grant is not sufficient merely because an earlier status call returned a state token. The command disables local pre-push hooks, push signing, and automatic tag following.
...
3. For fetch, inspect the validated `fetch_refspecs` returned by `git_list_remotes`; they show which branch or wildcard maps into the remote's own tracking namespace and therefore whether a later local ref observation can cover the branch. Decide explicitly whether local remote-tracking deletion via prune is intended.
...
- Runtime external-effect recovery in query-only mode performs a fresh local repository-state read and, while the remote configuration remains locally valid, captures a fresh local remote fingerprint. It does not contact the remote, compare that evidence into a definitive network outcome, or replay the operation. An `unknown` record therefore remains unknown until fresh authoritative evidence resolves it.
