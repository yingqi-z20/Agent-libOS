...
- `next_step=activate_skill` means at least one returned match is not the current loaded content. `next_step=use_loaded_skill` means every returned match is already loaded at the discovered hash. `next_step=refine_search` means the complete visible catalog had no match, so shorten or replace the terms once. `next_step=catalog_access_required` means no match was visible and catalog authority hid configured sources: stop discovery, report the visibility boundary, and request exact catalog read authority only when the Human goal truly requires registered Skills. Neither no-match result permits repeating an unchanged query.
