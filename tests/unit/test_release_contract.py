...
from scripts.generate_pypi_readme import generate as generate_pypi_readme
from tests.conftest import pytest_sessionfinish


ROOT = Path(__file__).resolve().parents[2]
REPOSITORY_BLOB_ROOT = (
    f"https://github.com/yingqi-z20/Agent-libOS/blob/v{__version__}/"
)
MUTABLE_REPOSITORY_BLOB_ROOT = (
    "https://github.com/yingqi-z20/Agent-libOS/blob/main/"
)
ACTION_PINS = {
    "actions/checkout": (
...
def _wheel_metadata(
    ...
    *,
    ...
    description = (
        ...
        "[Documentation](https://github.com/yingqi-z20/Agent-libOS/"
        f"blob/v{version}/README.md#documentation)\n"
    )
    return f"{metadata}\n{description}".encode()
...
def _write_release_pair(target: Path, *, version: str = "1.5.3") -> tuple[Path, Path]:
...
def test_root_lock_resolves_the_reviewed_mcp_sdk_v2_graph() -> None:
    ...
    assert packages["agent-libos"]["version"] == "1.5.3"
...
def test_repository_readme_uses_portable_relative_links() -> None:
    readme = (ROOT / "README.md").read_text(encoding="utf-8")
    destinations = re.findall(r"\[[^\]]+\]\(([^)]+)\)", readme)
    assert MUTABLE_REPOSITORY_BLOB_ROOT not in readme
    repository_links = [
        destination
    ...
    missing_targets = [
        destination
        for destination in repository_links
        if not (ROOT / destination.split("#", 1)[0]).is_file()
    ]
    assert missing_targets == []


def test_generated_pypi_readme_has_only_version_pinned_repository_links() -> None:
    readme = (ROOT / "README.pypi.md").read_text(encoding="utf-8")
    assert readme == generate_pypi_readme(ROOT)
    assert MUTABLE_REPOSITORY_BLOB_ROOT not in readme
    destinations = re.findall(r"\[[^\]]+\]\(([^)]+)\)", readme)
    relative = [
        destination
    ...
    assert missing_targets == []
    assert {
        f"{REPOSITORY_BLOB_ROOT}docs/index.md",
        f"{REPOSITORY_BLOB_ROOT}docs/tools_and_jit.md#on-demand-tool-skills",
        f"{REPOSITORY_BLOB_ROOT}docs/configuration_reference.md",
...
@pytest.mark.parametrize(
    ("replacement", "error"),
    (
        ("blob/main/", "mutable main-branch"),
        ("blob/v9.9.9/", "immutable tag v1.0.0"),
    ),
)
def test_wheel_validation_rejects_mutable_or_wrong_version_documentation_links(
    tmp_path: Path,
    replacement: str,
    error: str,
) -> None:
    metadata = _wheel_metadata("1.0.0").replace(b"blob/v1.0.0/", replacement.encode())
    wheel = _write_test_wheel(
        tmp_path / "agent_libos-1.0.0-py3-none-any.whl",
...
def test_release_status_contains_current_version_state_only() -> None:
    ...
    assert text.startswith("# Agent libOS 1.5.3 Status\n")
    forbidden = {
...
def test_release_status_bounds_unarchived_evidence_and_volatile_counts() -> None:
    ...
    assert "not Agent libOS 1.5.3 release evidence" in text
...
def test_installed_mcp_migration_smoke_reaches_current_store_schema() -> None:
    from scripts.smoke_mcp_extra import _migration_smoke

    evidence = _migration_smoke()

    assert evidence["from"] == 6
    assert evidence["to"] == 7
    assert evidence["backend"] == "sqlite"
    assert evidence["reopened"] is True
    assert evidence["reopened_schema_version"] == STORE_SCHEMA_VERSION
    assert len(str(evidence["v8_plan_sha256"])) == 64
...
def test_canonical_release_docs_cover_artifacts_and_clean_install_readback() -> None:
    readme = (ROOT / "README.md").read_text(encoding="utf-8")
    ...
    assert "dist/agent_libos-1.5.3-py3-none-any.whl" in development
    assert "dist/agent_libos-1.5.3.tar.gz" in development
    assert '"$RELEASE_WHEEL"' in releasing
    assert '"$RELEASE_SDIST"' in releasing
    assert "set(published) != set(canonical)" in releasing
    assert "downloaded file differs from canonical file" in releasing
    assert 'uv venv --python 3.11 "$RELEASE_READBACK_DIR/venv"' in releasing
    assert '"$RELEASE_READBACK_BIN/agent-libos" --db local demo' in releasing
    for entrypoint in EXPECTED_CONSOLE_SCRIPTS:
        assert f'"$RELEASE_READBACK_BIN/{entrypoint}" --help' in releasing
...
def test_gui_dependency_baseline_is_current_and_lockfile_aligned() -> None:
    ...
    assert locked_root["version"] == "1.5.3"
...
def test_local_release_recipe_keeps_the_release_only_environment() -> None:
    ...
    assert "uv sync --frozen --no-dev --group release" in recipe
    assert recipe.count(
        ".venv/bin/python scripts/check_release_artifacts.py dist"
    ) == 2
    assert "uv run python scripts/check_release_artifacts.py" not in recipe
...
def test_maintainer_governance_docs_match_current_release_boundaries() -> None:
    ...
    for required in (
        ...
        "must re-verify that the repository Security page",
        "Public publication is blocked if either check fails",
    ...
    assert "reports that no confidential vulnerability intake is enabled" not in releasing_normalized
...
def test_release_workflow_preserves_and_clean_installs_validated_artifacts() -> None:
    ...
    assert parsed["env"] == {
        "RELEASE_WHEEL": "dist/agent_libos-1.5.3-py3-none-any.whl",
        "RELEASE_SDIST": "dist/agent_libos-1.5.3.tar.gz",
